...
`dtlog` provides feedback about `data.table` operations. It redefines
the subsetting method `[.data.table` as well as several functions
exported by `data.table`, so it should be loaded **after** `data.table`,
otherwise there will be no output. A more explicit way to resolve
namespace conflicts is to use the `conflicted` package.
...
- `dtlog.display`:
  ...
  `NULL` (default) prints with
  [`message()`](https://rdrr.io/r/base/message.html). A list of
  functions sends the output to each of them. An empty list turns
  logging off, as does anything that is not a function, which is
  ignored.
...
- `dtlog.log_from_packages`:
  ...
  `FALSE` (default) only logs calls made from the global environment, so
  that `data.table` calls inside other packages stay silent.
...
- `dtlog.table_max_unique`:
  ...
  as possibly continuous instead of having its values listed. `Inf`
  lists every column.
