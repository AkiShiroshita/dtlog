# dttable() describes a single data.table and is base::table() for everything else
...
test_that("anything that is not a single data.table goes to base::table()", {
  expect_identical(dttable(DF$sex, DF$death), base::table(DF$sex, DF$death))
  expect_identical(dttable(SEX, DEATH), base::table(SEX, DEATH))
  expect_identical(dttable(SEX), base::table(SEX))
  expect_identical(dttable(DF), base::table(DF))
  expect_identical(dttable(as.data.frame(DF)), base::table(as.data.frame(DF)))
  expect_identical(dttable(factor(SEX, levels = c("m", "f", "x"))),
                   base::table(factor(SEX, levels = c("m", "f", "x"))))
})

test_that("the arguments of base::table() keep working", {
  expect_identical(dttable(SEX, DEATH, deparse.level = 2),
                   base::table(SEX, DEATH, deparse.level = 2))
  expect_identical(dttable(DF$sex, DF$death, deparse.level = 2),
                   base::table(DF$sex, DF$death, deparse.level = 2))
  expect_identical(dttable(SEX, DEATH, dnn = c("S", "D")),
                   base::table(SEX, DEATH, dnn = c("S", "D")))
  expect_identical(dttable(c(1, NA, 2), useNA = "ifany"),
                   base::table(c(1, NA, 2), useNA = "ifany"))
  expect_identical(dttable(c(1, 2, 2), exclude = 2), base::table(c(1, 2, 2), exclude = 2))
  # the names of the dimnames still come from the expressions as written
  expect_identical(names(dimnames(dttable(DF$sex, DF$death, deparse.level = 2))),
                   c("DF$sex", "DF$death"))
})
...
test_that("a two by two table built inside with() is base's", {
  ...
  two_by_two <- with(dat, dttable(estrogen = estrogen, dead = dead))
  ...
  expect_silent(with(dat, dttable(estrogen = estrogen, dead = dead)))
  # the same call on a data.frame, and with an argument of base::table()
  expect_identical(with(as.data.frame(dat), dttable(estrogen = estrogen, dead = dead)),
                   two_by_two)
  expect_identical(with(dat, dttable(estrogen = estrogen, dead = dead, useNA = "ifany")),
...
test_that("a data.table with any other argument goes to base::table() too", {
  ...
  expect_identical(dttable(dt, useNA = "ifany"), base::table(dt, useNA = "ifany"))
  # two tables are as much of an error as they are without dtlog
  expect_identical(
    tryCatch(dttable(dt, dt), error = conditionMessage),
    tryCatch(base::table(dt, dt), error = conditionMessage)
  )
})

test_that("base::table() calls stay visible and silent", {
  expect_true(withVisible(dttable(SEX))$visible)
  expect_silent(dttable(SEX, DEATH))
})

test_that("a single data.table is described", {
  env <- fresh_env()
  out <- quiet(dttable(env$DT))
  ...
  expect_false(withVisible(quiet(dttable(env$DT)))$visible)
})

test_that("the description is logged", {
  msgs <- dtlog_messages(quote(dttable(OTHER)))
  expect_identical(msgs[1L], "dttable: 2 rows and 2 columns")
  ...
  expect_true(all(startsWith(msgs[-1L], strrep(" ", nchar("dttable: ")))))
})
...
test_that("a factor carrying NA as a level is missing like any other NA", {
  ...
  out <- quiet(dttable(data.table::data.table(x)))
...
test_that("factors, dates and an empty table are described", {
  ...
  empty <- quiet(dttable(data.table::data.table()))
  expect_identical(nrow(empty), 0L)
  expect_identical(names(empty), c("Variable", "N_unique", "Unique_value"))
  expect_identical(dtlog_messages(quote(dttable(data.table::data.table()))),
                   "dttable: 0 rows and 0 columns")

  no_rows <- quiet(dttable(data.table::data.table(a = integer(0))))
...
test_that("the values are listed in the order the column sorts in", {
  ...
  values <- quiet(dttable(dt))[["Unique_value"]]
...
test_that("dtlog.table_max_unique moves the point where values stop being listed", {
  ...
  expect_identical(quiet(dttable(dt))[["Unique_value"]],
                   "5+ unique values \u2014 possibly continuous")
  options(dtlog.table_max_unique = 6)
  expect_identical(quiet(dttable(dt))[["Unique_value"]], "1; 2; 3; 4; 5")
  ...
  expect_identical(quiet(dttable(data.table::data.table(a = 1:22)))[["Unique_value"]],
                   paste(1:22, collapse = "; "))
  ...
  for (bad in list(NULL, "many", 0, 0.5, -1, NA, c(5, 10), TRUE)) {
    ...
    expect_identical(
      quiet(dttable(data.table::data.table(a = 1:20)))[["Unique_value"]],
...
test_that("a list column is printed like any other", {
  ...
  msgs <- dtlog_messages(quote(dttable(WITH_LIST)), env)
  expect_identical(msgs[1L], "dttable: 2 rows and 2 columns")
...
test_that("variables named in another script are described as they are shown", {
  ...
  out <- quiet(eval(quote(dttable(JP)), env))
  ...
  msgs <- dtlog_messages(quote(dttable(JP)), env)
  expect_length(msgs, 5L)
  expect_identical(msgs[1L], "dttable: 4 rows and 3 columns")
...
test_that("a long list of values is cut to 80 columns, not 80 characters", {
  ...
  value <- quiet(dttable(wide))[["Unique_value"]]
  expect_match(value, "\\.\\.\\.$")
...
test_that("values in any script are listed and cut to 80 columns", {
  ...
  for (nm in names(scripts)) {
    ...
    value <- quiet(dttable(dt))[["Unique_value"]]
...
test_that("a description in any script survives the transcript", {
  ...
  out <- quiet(eval(quote(dttable(MULTI)), env))
  expect_identical(out$Variable, names(env$MULTI))
  expect_false(anyNA(out$Unique_value))
  msgs <- dtlog_messages(quote(dttable(MULTI)), env)
  expect_length(msgs, 5L)
  lines <- table_transcript(eval(quote(dttable(MULTI)), env))
  expect_true(all(msgs %in% lines))
})

test_that("the description goes into the transcript", {
  env <- fresh_env()
  lines <- table_transcript(eval(quote(dttable(OTHER)), env))
  expect_true("> dttable(OTHER)" %in% lines)
  expect_true("dttable: 2 rows and 2 columns" %in% lines)
  expect_true(any(grepl("cyl +2 +4; 6$", lines)))
  expect_match(lines[length(lines)], "\\(one operation\\)$")
})

test_that("dttable() obeys dtlog_pause()", {
  env <- fresh_env()
  dtlog_pause()
  on.exit(dtlog_resume(), add = TRUE)
  expect_silent(paused <- eval(quote(dttable(OTHER)), env))
  dtlog_resume()
  expect_identical(canonical(paused), canonical(quiet(eval(quote(dttable(OTHER)), env))))
})

test_that("dttable() reads its argument once and changes nothing", {
  env <- fresh_env()
  dt <- env$DT
  before <- canonical(dt)
  ...
  loud(dttable(counted(dt)))
  expect_identical(calls, 1L)
  loud(dttable(counted(dt$cyl), counted(dt$gear)))
